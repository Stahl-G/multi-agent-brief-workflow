...
def test_claude_install_writes_user_command_and_agents(tmp_path, capsys):
    ...
    assert rc == 0
    output = capsys.readouterr().out
    assert "Installed /generate-brief" in output
    ...
    assert (target / "agents" / "mabw" / "scout.md").exists()
...
def test_claude_install_refuses_existing_non_mabw_file_without_force(tmp_path, capsys):
    ...
    assert rc == 1
    assert "Refusing to overwrite existing file without --force" in capsys.readouterr().out
