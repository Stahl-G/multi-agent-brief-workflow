...
然后在 Claude Code 中运行 `/generate-brief ../mabw-workspace`，由 subagents 完成 scout → screener → claim-ledger → analyst → editor → auditor 的完整流程。
...
> **生成简报的方式：**
> `/generate-brief <workspace>` 在 Claude Code 中编排完整 subagent 流程：
...
> ⚠️ **CLI-only 模式**：适用于 `init`、`doctor`、`audit`、`finalize` 等确定性 CLI 命令。
...
然后在 Claude Code 中运行 `/generate-brief my-workspace` 生成简报。
...
在 Claude Code 或 Codex 中输入 `/generate-brief ../mabw-workspace`，由 subagents 完成完整流程。
