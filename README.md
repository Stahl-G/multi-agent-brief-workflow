...
* `source_appendix.md`：可选的读者版来源附录，只使用正文实际引用的来源
...
> ```
> 去掉内部 `[src:CLAIM_ID]` 标记 → 生成 `brief.md` / `brief.docx` / 可选 `source_appendix.md` → 验证输出。

### 其他运行时

也支持 Claude Code、Codex、OpenCode。Clone 仓库并 `source .venv/bin/activate` 后，统一先用 CLI 生成 runtime handoff：
...
产物位于 `<workspace>/output/` 目录：`brief.md`、`brief.docx` 和可选 `source_appendix.md`；审计与账本文件位于 `<workspace>/output/intermediate/`。
