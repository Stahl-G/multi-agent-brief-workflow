...
v0.9.1 release 增加第一批实验性 Atomic Claim Graph 控制面，同时保留 MABW 作为实现血统和兼容面。runtime 命令、Python 包、workspace 格式、artifact 名称和 MABW-080 实验 ID 均不变。
...
* [v0.9.1 release notes](docs/releases/v0.9.1.md)：实验性 Atomic Claim Graph schema、coverage/type validation、Analyst/Editor contract boundary 和 reader-residue projection。MABW-080 operator sequence 仍见 [MABW-080 experiment guide](docs/experiments-080.md)。
...
当前 release baseline：v0.9.1

v0.9.1 release 增加实验性 Atomic Claim Graph schema、coverage/type validation、Analyst/Editor contract boundary 和 reader-residue projection。它不重命名 CLI、Python 包、workspace artifacts 或实验 ID，也仍未实现 Evidence Span Registry、Claim-Support Matrix、semantic proof 或 support-sufficiency gate。
