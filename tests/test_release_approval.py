...
def test_release_readiness_report_branding_blockers_must_match_event_metadata(tmp_path: Path) -> None:
    ...
    report = _json(report_path)
    assert report["branding_context"]["status"] == "blocked"
    assert report["branding_context"]["blockers"] == ["institution_branding_not_authorized:unauthorized"]
    replacement_blockers = ["institution_branding_not_authorized:synthetic_override"]
    report["branding_context"]["blockers"] = replacement_blockers
    report["blockers"] = replacement_blockers
    _write_json(report_path, report)
    assert validate_release_readiness_report_payload(report) is None
