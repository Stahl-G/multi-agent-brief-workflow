...
v0.9.4 release 在 Atomic Claim Graph、Evidence Span Registry 和 Claim-Support
Matrix 控制面之上，新增实验性 Semantic Assessment Report proposal surface，
同时保留 MABW 作为实现血统和兼容面。runtime 命令、Python 包、workspace 格式、
artifact 名称和 MABW-080 实验 ID 均不变。
...
copy、显式 atom-to-evidence support records、cross-artifact validation、从这些
显式记录投影出来的 gate/status 信号，以及 proposal-only Semantic Assessment
Report visibility。这些是可追溯、support-record 和 proposal 控制，不是语义支撑证明、
accepted support truth、adjudication queue、delivery gate、release eligibility，
也不是 support-sufficiency gate。

未发布的 Product OS surfaces 新增实验性 ReportSpec、ReportPack、
ReportTemplate 和 delivery/audit bundle manifest contracts。它们只是 metadata
和 projection 控制，不渲染 templates、不移动 artifacts、不绕过 gates、不交付
reports，也不授权发布。
...
当前 release baseline：v0.9.4

v0.9.4 release 新增实验性 Semantic Assessment Report proposal surface：schema、reference validation、proposal projection、status visibility 和 public-safe dogfood fixtures。它不重命名 CLI、Python 包、workspace artifacts 或实验 ID，不创建 support truth、adjudication queue、delivery gate、release authority、semantic proof、automatic support assessment、release eligibility 或 support-sufficiency gate。
